...
> **스택**: Python 3.12 · AWS Chalice · SQLModel · PostgreSQL (Supabase) / SQLite (로컬)  
> **원칙**: 관심사 분리(SoC) — Route → Service → DB 단방향 의존성

> ⚠️ **[구현 현황 주의]** 이 문서는 **설계 목표** 기준입니다.  
> 실제 구현 상태(Repository 레이어 미분리, Redis·MQTT 미구현 등)는  
> **[18_implementation_status.md](18_implementation_status.md)** 를 참고하세요.
>
> **v2.1 기준 실제 배포 환경**: AWS Lambda + API Gateway (프로덕션), Supabase PostgreSQL,  
> 시뮬레이터(`simulator.py`) → REST API 직접 전송 방식으로 MQTT 대체 중.
...
```
┌──────────────────┐     REST API      ┌────────────────────────────────────────────────┐
│  Vue3 Frontend   │ ◄────────────────► │                AWS API Gateway                 │
│  (Vercel)        │                   └──────────────────────┬─────────────────────────┘
└──────────────────┘                                          │ HTTP
                                        ┌─────────────────────▼─────────────────────────┐
┌──────────────────┐     REST API       │              AWS Lambda (Chalice)              │
│  simulator.py    │ ─────────────────► │                                                │
│  (IoT 대체)      │                   │  app.py                                        │
└──────────────────┘                   │  ├── Blueprint: auth_bp    (/auth/*)           │
                                        ...
                                        │                                                │
                                        │  chalicelib/                                   │
                                        │  ├── core/      (database, cors, jwt, pw)      │
                                        │  ├── models/    (SQLModel 테이블 정의)          │
                                        │  ├── schemas/   (Pydantic 응답 스키마)          │
                                        │  ├── services/  (비즈니스 로직, Session 직접)   │
...
└── chalicelib/
    ...
    │   ├── cors.py                 # ★ CORSConfig 팩토리 (ALLOWED_ORIGIN 환경변수 분기)
    │   ├── jwt_helper.py           # encode_jwt / decode_jwt (HS256)
    │   ├── password.py             # bcrypt 해싱
    │   └── exceptions.py           # APIException(code, message, status_code) 단일 클래스
    │   # ⚠️ 미구현: config.py(Pydantic BaseSettings), redis_client.py
    │
